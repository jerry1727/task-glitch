...
export function useTasks(): UseTasksState {
  ...
  // Initial load: public JSON -> fallback generated dummy (guarded for StrictMode re-invokes)
  useEffect(() => {
    ...
    (async () => {
      try {
        console.log('[Tasks] Initial load start');
        ...
        if (isMounted) {
          // fallback to seeded data if fetch fails
        ...
        if (isMounted) {
          setLoading(false);
          console.log('[Tasks] Initial load complete');
        }
      }
    })();
    return () => {
